...
// --- SIMULARE LOGIN PENTRU TESTARE (Șterge liniile astea după ce faci login-ul real) ---
// Dacă nu ai încă un sistem de login clienți, decomentează linia de mai jos pentru a testa cu clientul ID 1:
// $_SESSION['client_id'] = 1; 
// -------------------------------------------------------------------------------------

// 1. Verificare Autentificare Client
if (!isset($_SESSION['client_id'])) {
    // Dacă nu e logat, îl trimitem la pagina principală sau login
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $equipment_id = isset($_POST['vehicle']) ? (int)$_POST['vehicle'] : 0; // ID-ul mașinii selectate
    $service_type = isset($_POST['service']) ? trim($_POST['service']) : '';
    $date = isset($_POST['date']) ? trim($_POST['date']) : '';

    // Descriere scurtă pentru înregistrare
    ...
    if($equipment_id <= 0 || empty($date)) {
        ...
        // Inserare în tabela `appointments` cu status 'pending'
...
// 3. Preluare Vehiculele Clientului
