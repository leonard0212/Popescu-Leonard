...
// A. Appointments Today (status 'programata' or 'in_desfasurare' with today's date)
$admin_id = $_SESSION['admin_id'];
...
// B. Vehicles in Service (status 'in_desfasurare' regardless of date) - only for admin
...
// C. New Clients (Current Month) - only for admin
...
// D. ITPs Expiring (next 30 days) - only for admin's clients
...
// Service Name from session (or default)
$service_name = isset($_SESSION['service_name']) ? $_SESSION['service_name'] : 'Admin Service';

// Fetch new requests (status = 'pending') for dashboard display
// Fetch only new requests from admin's clients
        ...
        <main class="admin-content">
            <header class="admin-header animate-on-scroll">
                ...
                <nav class="user-info">
                    ...
                    <a href="admin_settings.php" class="btn btn-secondary">Setări</a>
                </nav>
            </header>
                                ...
                                <tr>
                                    ...
                                    <td class="actions">
                                        <form method="POST" style="display:inline">
                                            ...
                                            <button type="submit" class="btn btn-primary"><i class="fas fa-check"></i> Acceptă</button>
                                        </form>
                                        <form method="POST" style="display:inline; margin-left:8px;">
                                            <input type="hidden" name="update_appointment" value="1">
                                            <input type="hidden" name="appointment_id" value="<?php echo $r['id']; ?>">
                                            <input type="hidden" name="new_status" value="rejected">
                                            <button type="submit" class="btn btn-secondary"><i class="fas fa-times"></i> Respinge</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
