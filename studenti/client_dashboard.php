...
// --- SIMULARE LOGIN (Pentru testare rapidă, șterge după ce ai login real) ---
...
// Calcul zile rămase ITP
$itp_msg = "Nu ai vehicule cu ITP setat.";
$itp_car = "";
$itp_class = ""; // Clasa CSS pentru alertă
$days_left = 999;
...
if ($itp_data) {
    ...
    $days_left = (int)$interval->format('%r%a'); // %r dă semnul (negativ dacă a trecut)

    $itp_car = $itp_data['model'] . " (" . $itp_data['serial_number'] . ")";

    if ($days_left < 0) {
        $itp_msg = "EXPIRAT de " . abs($days_left) . " zile!";
        $itp_class = "alert-danger"; // Roșu
    } elseif ($days_left == 0) {
        $itp_msg = "Expiră AZI!";
        $itp_class = "alert-danger";
    } elseif ($days_left <= 30) {
        $itp_msg = "Expiră în $days_left zile";
        $itp_class = "alert"; // Galben/Portocaliu (din CSS-ul tău)
    } else {
        $itp_msg = "Valabil încă $days_left zile";
        $itp_class = "success"; // Verde
    }
}

// 4. Preluare Următoarea Programare (Revizie/Intervenție)
